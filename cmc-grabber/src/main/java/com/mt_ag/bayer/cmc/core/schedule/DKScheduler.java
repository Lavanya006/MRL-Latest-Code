...
import com.mt_ag.bayer.cmc.core.service.DKGrabberService;

@Component
public class DKScheduler extends DataSourceScheduler {

	private static final Logger LOGGER = LoggerFactory.getLogger(DKScheduler.class);

	private GrabbingJobService jobService;
	private DataSourceService dataSourceService;
	private DKGrabberService grabberService;
	private ThreadPoolTaskScheduler scheduler;
	private TaskExecutor taskExecutor;
	private DataSource dataSource;

	@Autowired
	public DKScheduler(GrabbingJobService jobService, DataSourceService dataSourceService,
			DKGrabberService grabberService, ThreadPoolTaskScheduler scheduler, TaskExecutor taskExecutor) {
		super();
	...
	private Runnable grabbingTask = () -> {
		Thread.currentThread().setName("DK");
		grabData();
	};

	@Override
	public void init() {
		this.dataSource = this.dataSourceService.findByName(DataSource.DK_DATA_SOURCE);
		scheduleNextExecution(this.jobService.getLastExecution(DataSource.DK_DATA_SOURCE) == null);
	}
...
//		shouldExecuteNow = true;
		...
		if (this.dataSourceService.isUp(dataSource, true)) {
			if (shouldExecuteNow) {
				LOGGER.info("Grabbing DK for the first time now " + LocalDateTime.now());
			...
			LOGGER.info("The next DK Grabbing will be on " + nextExecution);
		} else {
	...
	protected LocalDateTime computeNextExecution() {
		...
		LocalDateTime lastExecution = this.jobService.getLastExecution(DataSource.DK_DATA_SOURCE);
	...
	protected void tryAgainInOneHour() {
		LOGGER.info("The DK DataSource was down. Will try again in One Hour");
	...
	protected GrabbingJob createJob(LocalDateTime execution) {
		GrabbingJob job = new GrabbingJob(new Execution(execution, DataSource.DK_DATA_SOURCE), dataSource,
	...
	public void starManualExecution() {
		if (this.dataSourceService.isUp(dataSource, true)) {
			...
			LOGGER.info("Grabbing DK manually now " + LocalDateTime.now());
			taskExecutor.execute(grabbingTask);
			...
			LOGGER.info("Grabbing DK manually failed because the datasource is down.");
		}
	}
